...
import { analyzeCandidateForMission } from "@/lib/hr/qualification";
...
export async function POST(request: Request, context: RouteContext) {
  try {
    ...
    const result = await analyzeCandidateForMission({
      companyId,
      candidateId: id,
      applicationId,
      scoredBy: authUserId,
    ...
    const message = messageFromError(error, "Unable to analyze candidate");
