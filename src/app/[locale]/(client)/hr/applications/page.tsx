import { ArrowUpRight, Plus, Users, Zap, BarChart3, MapPin, Building2, RefreshCw, User } from "lucide-react";
...
export default async function ApplicationsPage({
  ...
  const applications = filtered.slice((currentPage - 1) * pageSize, currentPage * pageSize);
        ...
        <PageHeader
          ...
          meta={<div className="flex items-center gap-4">
            ...
            <span className="font-bold text-foreground/50 tracking-tight">
              <span className="text-pink-600">{total}</span> {total > 1 ? "Application Projects" : "Application Project"}
            </span>
          </div>}
          actions={<ActionLink href="/hr/applications/new" icon={Plus} variant="pink">New application</ActionLink>}
        />
      </div>
              ...
              <div className="relative flex flex-1 items-center gap-8">
                ...
                <div className="flex items-center gap-12 shrink-0 pr-4">
                  <div className="flex flex-col items-center w-20">
                    <span className="text-[9px] uppercase tracking-[0.2em] text-foreground/25 font-black mb-1.5">Applicants</span>
                    <div className="flex items-center gap-1.5 font-bold text-foreground transition-transform group-hover:scale-105">
                      <Users className="size-3.5 text-pink-500/60" />
